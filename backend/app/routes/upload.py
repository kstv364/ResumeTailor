...
async def upload_document(body: UploadRequest):
    try:
        ...
        embedder.store_resume(body.id, text)
        version_id = versioning.create_version(body.id, latex, job_desc_id=None)
        return {"status": "uploaded", "resume_id": body.id, "version_id": version_id}
